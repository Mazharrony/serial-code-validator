...
if (isset($_POST['sv_generate_codes']) && check_admin_referer('sv_generate_codes_nonce', 'sv_generate_codes_nonce_field')) {
    $params = array(
        ...
        'warranty_months' => isset($_POST['warranty_months']) ? sanitize_text_field($_POST['warranty_months']) : '',
        'status' => isset($_POST['status']) ? sanitize_text_field($_POST['status']) : 'active',
        'one_time_use' => isset($_POST['one_time_use']) ? 1 : 0,
        'code_length' => isset($_POST['code_length']) ? intval($_POST['code_length']) : 12,
        'code_prefix' => isset($_POST['code_prefix']) ? sanitize_text_field($_POST['code_prefix']) : '',
        'code_suffix' => isset($_POST['code_suffix']) ? sanitize_text_field($_POST['code_suffix']) : '',
        'code_format' => isset($_POST['code_format']) ? sanitize_text_field($_POST['code_format']) : 'alphanumeric'
    );
...
<div class="wrap sv-generate-codes">
    <h1><?php _e('Generate Codes', 'serial-validator'); ?></h1>
        ...
        <form method="post" action="" id="sv-generate-form">
            ...
            <div class="sv-generate-sections">
                ...
                <div class="sv-generate-section">
                    <h2><?php _e('Generation Settings', 'serial-validator'); ?></h2>
                    
                    <table class="form-table">
                        <tr>
                            <th scope="row">
                                <label for="quantity"><?php _e('Quantity', 'serial-validator'); ?> <span class="required">*</span></label>
                            </th>
                            <td>
                                <input type="number" name="quantity" id="quantity" class="regular-text" min="1" max="10000" value="100" required>
                                <p class="description"><?php _e('Number of codes to generate (1-10,000)', 'serial-validator'); ?></p>
                            </td>
                        </tr>
                        
                        <tr>
                            <th scope="row">
                                <label for="code_format"><?php _e('Code Format', 'serial-validator'); ?></label>
                            </th>
                            <td>
                                <select name="code_format" id="code_format" class="regular-text">
                                    <option value="alphanumeric"><?php _e('Alphanumeric (A-Z, 0-9)', 'serial-validator'); ?></option>
                                    <option value="numeric"><?php _e('Numeric (0-9)', 'serial-validator'); ?></option>
                                    <option value="alphabetic"><?php _e('Alphabetic (A-Z)', 'serial-validator'); ?></option>
                                </select>
                            </td>
                        </tr>
                        ...
                        <tr>
                            <th scope="row">
                                <label for="code_length"><?php _e('Code Length', 'serial-validator'); ?></label>
                            </th>
                            <td>
                                <input type="number" name="code_length" id="code_length" class="regular-text" min="6" max="20" value="12">
                                <p class="description"><?php _e('Total length including prefix and suffix (6-20 characters)', 'serial-validator'); ?></p>
                            </td>
                        </tr>
                        
                        <tr>
                            <th scope="row">
                                <label for="code_prefix"><?php _e('Prefix', 'serial-validator'); ?></label>
                            </th>
                            <td>
                                <input type="text" name="code_prefix" id="code_prefix" class="regular-text" maxlength="5" placeholder="e.g., SN-">
                                <p class="description"><?php _e('Optional prefix for all codes (max 5 characters)', 'serial-validator'); ?></p>
                            </td>
                        </tr>
                        
                        <tr>
                            <th scope="row">
                                <label for="code_suffix"><?php _e('Suffix', 'serial-validator'); ?></label>
                            </th>
                            <td>
                                <input type="text" name="code_suffix" id="code_suffix" class="regular-text" maxlength="5" placeholder="e.g., -2024">
                                <p class="description"><?php _e('Optional suffix for all codes (max 5 characters)', 'serial-validator'); ?></p>
                            </td>
                        </tr>
                    </table>
                </div>
                ...
                <div class="sv-generate-section">
                    <h2><?php _e('Code Properties', 'serial-validator'); ?></h2>
                    
                    <table class="form-table">
                        <tr>
                            <th scope="row">
                                <label for="product_name"><?php _e('Product Name', 'serial-validator'); ?> <span class="required">*</span></label>
                            </th>
                            <td>
                        ...
                        <tr>
                            <th scope="row">
                                <label for="batch"><?php _e('Batch Number', 'serial-validator'); ?></label>
                            </th>
                            <td>
                                <input type="text" name="batch" id="batch" class="regular-text" placeholder="e.g., BATCH-2024-01">
                                <p class="description"><?php _e('Optional batch identifier for tracking', 'serial-validator'); ?></p>
                            </td>
                        </tr>
                        
                        <tr>
                            <th scope="row">
                                <label for="expiry_date"><?php _e('Expiry Date', 'serial-validator'); ?></label>
                            </th>
                            <td>
                                <input type="date" name="expiry_date" id="expiry_date" class="regular-text">
                                <p class="description"><?php _e('Optional expiry date for codes', 'serial-validator'); ?></p>
                            </td>
                        </tr>
                        
                        <tr>
                            <th scope="row">
                                <label for="warranty_months"><?php _e('Warranty (Months)', 'serial-validator'); ?></label>
                            </th>
                            <td>
                                <input type="number" name="warranty_months" id="warranty_months" class="regular-text" min="0" max="120" placeholder="e.g., 12">
                                <p class="description"><?php _e('Warranty period in months (max 120)', 'serial-validator'); ?></p>
                            </td>
                        </tr>
                        
                        <tr>
                            <th scope="row">
                                <label for="status"><?php _e('Status', 'serial-validator'); ?></label>
                            </th>
                            <td>
                                <select name="status" id="status" class="regular-text">
                                    <option value="active"><?php _e('Active', 'serial-validator'); ?></option>
                                    <option value="blocked"><?php _e('Blocked', 'serial-validator'); ?></option>
                                </select>
                            </td>
                        </tr>
                        ...
                        <tr>
                            <th scope="row">
                                <label for="one_time_use"><?php _e('One-Time Use', 'serial-validator'); ?></label>
                            </th>
                            <td>
                                <label>
                                    <input type="checkbox" name="one_time_use" id="one_time_use" value="1" checked>
                                    <?php _e('Codes can only be verified once', 'serial-validator'); ?>
                                </label>
                            </td>
                        </tr>
                    </table>
            ...
            <div class="sv-generate-section sv-preview-section">
                <h2><?php _e('Code Preview', 'serial-validator'); ?></h2>
                <div class="sv-code-preview">
                    <p class="description"><?php _e('Sample codes based on your settings:', 'serial-validator'); ?></p>
...
jQuery(document).ready(function($) {
    ...
    $('#generate-download-btn').on('click', function(e) {
        ...
        if (!$('#product_name').val()) {
            alert('<?php _e('Please enter a product name.', 'serial-validator'); ?>');
            $('#product_name').focus();
            return;
        }
        ...
        if (!quantity || quantity < 1 || quantity > 10000) {
            alert('<?php _e('Please enter a valid quantity (1-10,000).', 'serial-validator'); ?>');
            $('#quantity').focus();
            return;
        }
